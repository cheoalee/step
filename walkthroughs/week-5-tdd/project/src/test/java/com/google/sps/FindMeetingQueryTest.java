...
public final class FindMeetingQueryTest {
  ...
  private static final int TIME_1115AM = TimeRange.getTimeInMinutes(11, 15);
  ...
  @Test
  public void overlappingEvents_twoAttendees() {
  ...
  @Test
  public void overlappingEvents_threeAttendees() {
  ...
  @Test
  public void everyAttendeeIsConsidered_noOptionalFree() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Optional: |--------------C--------------|
    ...
    @Test
  public void everyAttendeeIsConsidered_optionalFree() {
    // Have each person have different events. We should see two options because each person has
    // split the restricted times.
    //
    // Optional:             |--C--|
  ...
  @Test
  public void justEnoughRoom_noOptionalFree() {
    // Have one person, but make it so that there is just enough room at one point in the day to
    // have the meeting.
    //
    // Optional:       |B|
  ...
  @Test
  public void optionalOnly_severalGaps() {
    // Have each person have different events. We should see six options because each person has
    // split the restricted times.
    //
    // Events  :   |A| |B|  |-A-|     |B|  |A|
    // Day     : |------------------------------|
    ...
    Collection<Event> events = Arrays.asList(
        ...
        new Event("Event 3", TimeRange.fromStartDuration(TIME_0900AM, DURATION_30_MINUTES),
            Arrays.asList(PERSON_A)),
        new Event("Event 4", TimeRange.fromStartDuration(TIME_1030AM, DURATION_15_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 5", TimeRange.fromStartDuration(TIME_1100AM, DURATION_15_MINUTES),
            Arrays.asList(PERSON_A)));
 
    MeetingRequest request =
        new MeetingRequest(NO_ATTENDEES, DURATION_15_MINUTES);
    ...
    Collection<TimeRange> expected =
        Arrays.asList(TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0800AM, false),
            ...
            TimeRange.fromStartEnd(TIME_0930AM, TIME_1030AM, false),
            TimeRange.fromStartEnd(TIME_1045AM, TIME_1100AM, false),
            TimeRange.fromStartEnd(TIME_1115AM, TimeRange.END_OF_DAY, true));
 
    Assert.assertEquals(expected, actual);
  }

  @Test
  public void optionalOnly_noGaps() {
    // Have each person have different events. We should see six options because each person has
    // split the restricted times.
    //
    // Events  : |A||B||-----A-----||---B---||A|
    ...
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0815AM, false),
            Arrays.asList(PERSON_A)),
        new Event("Event 2", TimeRange.fromStartDuration(TIME_0815AM, DURATION_15_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 3", TimeRange.fromStartDuration(TIME_0830AM, DURATION_2_HOUR),
            Arrays.asList(PERSON_A)),
        new Event("Event 4", TimeRange.fromStartDuration(TIME_1030AM, DURATION_15_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 5", TimeRange.fromStartDuration(TIME_1045AM, TimeRange.END_OF_DAY),
            Arrays.asList(PERSON_A)));
